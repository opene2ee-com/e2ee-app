...
//
// Sprint 22 — Pure-Dart scoring engine for e2ee-ap-v2 session aggregates.
//
// The Skorlar screen reads the recent completed sessions from the
// backend's `GET /api/v1/sessions?status=completed` endpoint (Sprint
// 23+ when `auth_service` + `telemetry_service` ship) and feeds
// each session's `summary_stats` block into
// `SessionScoreCalculator.compute(...)`. The result is the
// per-session score the UI shows in the `SessionScoreCard`.
//
// Audit invariants (Sprint 11.0C origin):
//   S62 — `compute()` is a pure function (no I/O, no time-source
...
// sampling side (Sprint 22.10), not here.

import 'dart:math' as math;

/// SessionTelemetry is the input to [SessionScoreCalculator.compute].
/// All fields except the three required identifiers have defaults so
/// the "session in progress" case (the Skorlar screen surfaces a
/// placeholder card for those) can build one trivially.
class SessionTelemetry {
  ...
  /// Sprint 23+) was valid. Computed by the backend in
  /// `summary_stats`.
