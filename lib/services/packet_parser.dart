...
//
// Sprint 22 — Dart port of (a subset of) gopacket. Pure-Dart, no
...
// layer decoders) for the very small slice e2ee-ap-v2 actually needs:
//
...
// at this stage. The `tlsFingerprint` field is reserved for a future
// sprint when the Dart side will be allowed to read a TINY bounded
// prefix of the TCP payload to fingerprint the SNI hash. For now it
// is always `null`.
//
// The [SampledPacket] class is the wire-format type the Kotlin side
// produces (Sprint 22.10 — wirebare-kernel PacketInterceptor /
// IImportantEventListener hook). Each entry is a
// `Map<String, Object?>` whose keys match `SampledPacket.toJson()`
// verbatim. `fromBytes(raw)` is the inverse of the Kotlin extractor
// for cases where the Dart side needs to re-parse a buffer.
//
...
/// [PacketParser.parse]; the [tlsFingerprint] field is reserved
/// for a future sprint and is always `null` in 22.x.
class ParsedPacket {
  ParsedPacket({
  ...
  /// Reserved. Sprint 22.x: always `null`. Future sprint: populated
...
class PacketParser {
  ...
  static ParsedPacket? _parseIpv6(Uint8List raw) {
    ...
    // we deliberately drop both; not surfaced to telemetry in 22.x.)
    ...
    // no-extension case plus a small walk past Hop-by-Hop /
    // Routing / Fragment / ESP / AH (up to 4 levels). Anything
    // more exotic returns null rather than risk mis-parsing the
    // L4. Documented limitation matching the wirebare-kernel
    // PacketDispatcher.
...
// ═══ Sprint 22.10 — SampledPacket (wire format) ═══
//
// Wire format produced by the Kotlin PacketInterceptor
// (Sprint 22.10). Each entry is a `Map<String, Object?>` whose keys
// match `SampledPacket.toJson()` verbatim. The `fromBytes` factory
// re-parses a raw `Uint8List` buffer for the rare case the Dart
// side wants to inspect a single packet end-to-end (e.g. debug
// pipeline). The two factories feed the same field set; the
// `toJson` round-trips to the MethodChannel wire format.
//
...
class SampledPacket {
  ...
  /// Wire format — matches the keys the Kotlin PacketInterceptor
  /// emits (Sprint 22.10). The `packetStream` MethodChannel event
  /// (Sprint 22.11) feeds this exact shape into the Dart side.
  Map<String, Object?> toJson() => {
        'version': version,
        'protocol': protocol,
        'protocolNumber': protocolNumber,
