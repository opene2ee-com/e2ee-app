"""Self-test for check_app_build_gradle_syntax_v2.

Per Architect brief: "self-checks (negative test: revert + audit
finds 4 FAIL)". We construct 5 broken files, each violating exactly
one of the S1-S5 sub-checks, run the check against each, and
assert it finds the expected failure.
"""
import sys
...
cases = [
    ("PASS (Sprint 9.6.6 fixed file)", case_pass, []),
    ("S1+S5 fail (Sprint 9.6.5 broken state: no real import + fully-qualified usage)", case_s1_fail, ["S1 fail", "S5 fail"]),
    ("S2 fail (missing JvmTarget import)", case_s2_fail, ["S2 fail"]),
    ("S3 fail (deprecated kotlinOptions present)", case_s3_fail, ["S3 fail"]),
    ("S4 fail (missing new kotlin compilerOptions block)", case_s4_fail, ["S4 fail"]),
    ("S5 fail (fully-qualified java.util.Properties())", case_s5_fail, ["S5 fail"]),
]

failed = []
for name, code, expected in cases:
    actual = run_check(code)
